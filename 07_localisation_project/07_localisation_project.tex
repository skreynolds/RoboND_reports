...
\captionof{figure}{We see that the uncertainty of where the robot is located increases as it moves. Uncertainty is indicated by highly dispersed points.}
\end{minipage}
...
\begin{minipage}{0.45\textwidth}
\centering
\captionof{table}{Descriptive measurement summary for the Alternative model}
\begin{tabular}{lc}
\toprule
\textbf{Dimension} & \textbf{Measurement ($\si{\meter}$)} \\
\midrule
...
\captionof{figure}{Alternative model before the localisation process has commenced. Note the distribution of green pose arrows around the robot indicating a high level of pose uncertainty.}
\end{minipage}
...
\captionof{figure}{Alternative model navigating the environment as \texttt{amcl} localises the robot. The distribution of green arrows is beginning to converge on the correct pose.}
\end{minipage}
...
\captionof{figure}{Alternative model at the goal location. The tight clustering of green arrows indicates particle convergence on true robot location for \texttt{amcl}}
\end{minipage}
